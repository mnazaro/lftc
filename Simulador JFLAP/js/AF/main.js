...
function verifyStartExists() {
    if(FA.startState == null) {
        alert("ERRO: Não foi definido um estado inicial");
        return false;
    }
    setStatus("Start");
    return true;
...
function stepButtonHandler() {
    ...
    if(FA.inputIndex >= FA.input.length) {
        ...
        if(accepted) {
            setStatus("String Aceita")
            for (let i = 0; i < stepDisplay.childNodes.length; i++) {
                stepDisplay.childNodes[i].style.color = "green";
            }
        }
        else {
            setStatus("String Rejeitada")
            for (let i = 0; i < stepDisplay.childNodes.length; i++) {
                stepDisplay.childNodes[i].style.color = "red";
            }
        }
        return;
    }
    ...
    stepDisplay.childNodes[FA.inputIndex].style.color = "#007bff";
...
function newFA() {
    if(confirm("Tem certeza de que deseja iniciar um novo AF?")) {
        FA.newFA();
        scene.drawAll();
    }
}
...
function load(e) {
    if(confirm("Tem certeza de que deseja carregar um novo ?\nQualquer trabalho não salvo será perdido.")) {
...
//Definindo um autômato Finito para testar o simulador:
